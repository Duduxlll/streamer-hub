...
func _ready() -> void:
	...
	ensure_marble_pool(MAX_MARBLES)
...
func set_mode(mode):
	...
	if _mode == State.MODE_MARBLE:
		...
		if start_a_new_race:
			...
			_rotation_camera.distance_to_target = 95.0
		...
		_rotation_camera.distance_to_target = 185.0
...
func open_site_waiting() -> void:
	...
	if _race.get_child_count() == 0:
		if _race.has_method(&"configure_for_marble_count"):
			_race.configure_for_marble_count(max(len(_site_players), 100))
