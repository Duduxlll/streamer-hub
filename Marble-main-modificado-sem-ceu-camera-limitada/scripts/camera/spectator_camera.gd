...
func _physics_process(_delta: float) -> void:
	# O Main chama handle_keyboard_camera(delta) diretamente.
	# Assim funciona melhor quando a UI/ranking está com foco.
	pass


func rotate_yaw_by_mouse(relative_x: float) -> void:
	# Mouse segurado gira só na horizontal.
	# Ignora movimento vertical para não virar diagonal/de cabeça para baixo.
	_yaw += -relative_x * sensitivity * 9.0
	_apply_rotation()
...
func handle_keyboard_camera(delta: float) -> void:
	...
	# A/D = movimento lateral, não gira.
...
func clamp_camera_bounds() -> void:
	# Limite amplo para não fugir do mapa e não renderizar longe demais.
	# Mantém a pista inteira acessível.
	global_position.x = clampf(global_position.x, -420.0, 420.0)
	global_position.z = clampf(global_position.z, -420.0, 420.0)
	global_position.y = clampf(global_position.y, 3.0, 180.0)
