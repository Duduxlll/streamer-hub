...
var _camera_mouse_drag_enabled := false
var _camera_mouse_sensitivity := 0.006
...
func get_active_game_camera() -> Camera3D:
	var viewport_camera := get_viewport().get_camera_3d()
	if viewport_camera != null:
		return viewport_camera

	if _marble_camera != null and _marble_camera.is_inside_tree():
		return _marble_camera

	if _rotation_camera != null and _rotation_camera.is_inside_tree():
		return _rotation_camera

	if _free_camera != null and _free_camera.is_inside_tree():
		return _free_camera

	return null


func copy_current_camera_to_free_camera() -> void:
	var active_camera := get_active_game_camera()
	if active_camera != null and _free_camera != null:
		if _free_camera.has_method("set_from_camera"):
			_free_camera.set_from_camera(active_camera)
		else:
			_free_camera.global_transform = active_camera.global_transform


func start_free_camera_from_current_view() -> void:
	copy_current_camera_to_free_camera()
...
func _unhandled_input(event):
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT or event.button_index == MOUSE_BUTTON_RIGHT:
			_camera_mouse_drag_enabled = event.pressed
			Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
			get_viewport().set_input_as_handled()
			return

	if event is InputEventMouseMotion:
		if _camera_mouse_drag_enabled:
			handle_camera_mouse_yaw(event.relative.x)
			Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
			get_viewport().set_input_as_handled()
			return
	...
	if event is InputEventKey:
		if event.pressed:
			match event.keycode:
				KEY_W, KEY_A, KEY_S, KEY_D, KEY_UP, KEY_DOWN, KEY_LEFT, KEY_RIGHT, KEY_Q, KEY_Z, KEY_E, KEY_CTRL, KEY_SHIFT:
					if _mode == State.MODE_MARBLE:
						start_free_camera_from_current_view()
						return
...
func handle_camera_mouse_yaw(relative_x: float) -> void:
	if _mode != State.MODE_MARBLE:
		return

	if not _free_camera_enabled or not _free_camera.is_inside_tree():
		start_free_camera_from_current_view()

	if _free_camera != null and _free_camera.has_method("rotate_yaw_by_mouse"):
		_free_camera.rotate_yaw_by_mouse(relative_x)


func is_free_camera_key_pressed() -> bool:
	return (
...
func handle_free_camera_keyboard(delta: float) -> void:
	if _mode != State.MODE_MARBLE:
		return

	# Lê o teclado direto a cada frame.
	# Isso é mais confiável no Web/iframe do que depender de _unhandled_input.
	if is_free_camera_key_pressed():
		if not _free_camera_enabled or not _free_camera.is_inside_tree():
			start_free_camera_from_current_view()

	if _free_camera_enabled and _free_camera.is_inside_tree():
		if _free_camera.has_method("handle_keyboard_camera"):
			_free_camera.handle_keyboard_camera(delta)


func _process(delta):
	_time += delta
	handle_free_camera_keyboard(delta)
