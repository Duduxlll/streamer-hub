...
func _ready() -> void:
	...
	var display_name: String = str(_marble.get_marble_name()).to_upper()
	if len(display_name) > 22:
		display_name = display_name.substr(0, 21) + "…"
	_name_label.set_text(display_name)
...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			...
			if main_node != null and main_node.has_method("follow_marble_from_ranking"):
				main_node.follow_marble_from_ranking(_marble)
