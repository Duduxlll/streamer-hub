...
var _camera_drag_sensitivity := 0.006
...
func is_camera_drag_available() -> bool:
	return _mode == State.MODE_MARBLE


func rotate_camera_by_drag(relative: Vector2) -> void:
	var cam := get_active_game_camera()
	if cam == null:
		return

	var yaw: float = -relative.x * _camera_drag_sensitivity
	var pitch: float = -relative.y * _camera_drag_sensitivity

	# Gira no mundo para esquerda/direita e no eixo local para cima/baixo.
	cam.rotate_y(yaw)
...
func _unhandled_input(event):
	...
	if event is InputEventMouseMotion:
		if _camera_drag_enabled:
			rotate_camera_by_drag(event.relative)
			get_viewport().set_input_as_handled()
			return
