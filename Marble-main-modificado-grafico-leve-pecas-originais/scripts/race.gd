...
var _track_scale: float = 3.6
var _step_count: int = 24

func configure_for_marble_count(amount: int) -> void:
	# Versão live: mapa maior para caber bolinhas, mas corrida mais curta.
	# Menos peças/estruturas = define vencedor mais rápido.
	if amount <= 80:
		_track_scale = 3.2
		_step_count = 12
	elif amount <= 120:
...
func optimize_piece_visibility(piece: Piece) -> void:
	for child in piece.get_children():
		if child is MeshInstance3D:
			child.visibility_range_end = 1400
