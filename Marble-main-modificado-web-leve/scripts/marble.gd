...
func _ready() -> void:
	...
	var x_ray_material := _ball_mesh.get_active_material(0).duplicate() as StandardMaterial3D
	x_ray_material.set_albedo(color)
	var toon_material = x_ray_material.get_next_pass()
	if toon_material is StandardMaterial3D:
		toon_material = toon_material.duplicate() as StandardMaterial3D
...
func _start() -> void:
	...
	# Set collision mask
	# Versão Web leve:
	# colisão com mapa/props fica ativa,
	# colisão bolinha-com-bolinha fica desligada para reduzir CPU.
