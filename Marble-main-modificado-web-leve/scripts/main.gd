...
# Garante uma piscina grande de bolinhas sem precisar duplicar manualmente na cena.
func ensure_marble_pool(amount: int) -> void:
	amount = clampi(amount, 1, MAX_MARBLES)

	# Remove bolinhas extras para não deixar física/malha escondida pesando no Web.
	...
	var current_count := _marble_pool.get_child_count()
...
func set_mode(mode):
	...
	if _mode == State.MODE_MARBLE:
		...
		_rotation_camera.distance_to_target = 60.0
...
func open_site_waiting() -> void:
	...
	# Na versão Web leve, não gera pista enquanto está aguardando jogadores.
	# Isso evita carregar mapa/física antes da hora.
...
func finish_site_race() -> void:
	...
	_site_subtitle.text = "Resultado dos ganhadores"
...
func set_speed_multiplier(multiplier: int) -> void:
	if OS.has_feature("web") or OS.get_name() == "Web" or OS.get_name() == "HTML5":
		_speed_multiplier = clampi(multiplier, 1, 2)
	else:
